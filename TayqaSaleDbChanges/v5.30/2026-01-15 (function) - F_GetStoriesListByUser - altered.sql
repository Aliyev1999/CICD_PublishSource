...
begin
    ...
    from WPM_Attachment Attachments with (nolock)
             ...
             OUTER APPLY (SELECT TOP 1 * FROM MD_PhotoLike WITH (NOLOCK)
                         WHERE ReferenceId = Attachments.Id AND SourceType = 2
                         ORDER BY CreationTime DESC) Likes
             OUTER APPLY (SELECT TOP 1 * FROM MD_PhotoStar WITH (NOLOCK)
                         WHERE ReferenceId = Attachments.Id AND SourceType = 2
                         ORDER BY CreationTime DESC) Stars
             OUTER APPLY (SELECT TOP 1 * FROM MD_PhotoComment WITH (NOLOCK)
                         WHERE ReferenceId = Attachments.Id AND SourceType = 2
                         ORDER BY CreationTime DESC) Comments
    where Users.Id = @UserId
      and Attachments.Type = 3
      and Clients.Firm = @firm
      and Attachments.CreatedDate >= cast(Getdate() as Date)
      and taskactions.ActionType not in (2,49,50)
    ...
    from CHL_Attachment Attachments with (nolock)
             ...
             left join (select distinct ImageUrl
                       ...
                       on StoryLog.ImageUrl collate Azeri_Latin_100_CI_AS = Attachments.SecureUrl
             OUTER APPLY (SELECT TOP 1 * FROM MD_PhotoLike WITH (NOLOCK)
                         WHERE ReferenceId = Attachments.Id AND SourceType = 1
                         ORDER BY CreationTime DESC) Likes
             OUTER APPLY (SELECT TOP 1 * FROM MD_PhotoStar WITH (NOLOCK)
                         WHERE ReferenceId = Attachments.Id AND SourceType = 1
                         ORDER BY CreationTime DESC) Stars
    ...
    from OP_FileUploadLog Files with (nolock)
             ...
             OUTER APPLY (SELECT TOP 1 * FROM MD_PhotoLike WITH (NOLOCK)
                         WHERE ReferenceId = Files.Id AND SourceType in (3, 5)
                         ORDER BY CreationTime DESC) Likes
             OUTER APPLY (SELECT TOP 1 * FROM MD_PhotoStar WITH (NOLOCK)
                         WHERE ReferenceId = Files.Id AND SourceType in (3, 5)
                         ORDER BY CreationTime DESC) Stars
