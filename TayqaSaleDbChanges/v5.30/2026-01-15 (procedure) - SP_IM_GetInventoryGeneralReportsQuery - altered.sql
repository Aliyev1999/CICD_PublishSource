...
create or alter procedure [dbo].[SP_IM_GetInventoryGeneralReportsQuery](
															  @firm int null,
                                                              @tigerId int null,
...
insert
into @InProgressTable (InventoryId, ClientCode, ClientName, OrderNo)
...
insert into @InventoryData (InventoryId, ClientCode, ClientName, ClientId)
select coalesce(InProgress.InventoryId, Others.InventoryId) as InventoryId,
...
from @InProgressTable as InProgress
         full outer join (select Inventory.Id   AS InventoryId,
                          ...
                          from IM_Inventory Inventory with (nolock)
                                   left join MD_Client Client with (nolock)
                                             on Client.TigerId = Inventory.ClientTigerId
                                                 and Client.Firm = Inventory.Firm) Others
...
from @InventoryData as List
         inner join
     IM_Inventory Inventory with (nolock) on Inventory.Id = List.InventoryId
         inner join
     MD_Firm Firm with (nolock) on Firm.Nr = Inventory.Firm and Firm.IsActive = 1
         inner join
     MD_Item Item with (nolock) on Item.TigerId = Inventory.TigerId and Inventory.Firm = Item.Firm
         inner join
     MD_Division Division with (nolock) on Division.Nr = Inventory.DivisionNr and Division.Firm = Inventory.Firm
         left join
     AbpUsers CreatorUser with (nolock) on Inventory.CreatorUserId = CreatorUser.Id
         left join
     MD_Salesman Salesman with (nolock) on Salesman.TigerId = Inventory.ResponsiblePerson and Salesman.Firm = Inventory.Firm and Salesman.IsDeleted = 0
         left join
     MD_Salesman Manager with (nolock) on Manager.TigerId = Inventory.ResponsiblePerson2 and Manager.Firm = Inventory.Firm and Manager.IsDeleted = 0
where 1 = 1
...
if @tigerId is not null 
	set @Query = concat(@Query, ' and Item.TigerId = @tigerId ')

if @clientTigerId is not null 
	set @Query = concat(@Query, ' and List.ClientId = @clientTigerId ')

if @registrationNr is not null 
	set @Query = concat(@Query, ' and (Inventory.RegistrationNr like ''%''+@registrationNr+''%'')')

if @warehouseNr is not null 
	set @Query = concat(@Query, ' and Inventory.WarehouseNr = @warehouseNr ')

if @divisionNr is not null 
	set @Query = concat(@Query, ' and Inventory.DivisionNr = @divisionNr ')

if @departmentNr is not null 
	set @Query = concat(@Query, ' and Inventory.DepartmentNr = @departmentNr ')

if @warehouseNr is not null 
	set @Query = concat(@Query, ' and Inventory.WarehouseNr = @warehouseNr ')

if @itemNameOrCode is not null 
	set @Query = concat(@Query, ' and (Item.Code like ''%''+@itemNameOrCode+''%'' or Item.Name like ''%''+@itemNameOrCode+''%'')')

if @clientNameOrCodeGroupCode is not null
    set @Query =
            concat(@Query, ' and (List.ClientCode like ''%''+@clientNameOrCodeGroupCode+''%'' or List.ClientName like ''%''+@clientNameOrCodeGroupCode+''%'')')

if @startDate is not null 
	set @Query = concat(@Query, ' and Inventory.RegistrationDate >= @startDate')

if @endDate is not null 
	set @Query = concat(@Query, ' and Inventory.RegistrationDate <= @endDate')

if @inventoryStatuses is not null
    set @Query = concat(@Query, ' AND (Inventory.Status IN (SELECT LTRIM(Value) FROM F_SplitList(''', @inventoryStatuses, ''',', ''','')))')
if @producerCode is not null
    set @Query = concat(@Query, ' and (Item.ProducerCode like ''%''+@producerCode+''%'' or Item.ProducerName like ''%''+@producerCode+''%'')')

if @groupCode is not null 
	set @Query = concat(@Query, ' and (Item.GroupCode like ''%''+@groupCode+''%'' or Item.GroupName like ''%''+@groupCode+''%'')')
...
if @responsiblePersons is not null
    set @Query = concat(@Query, ' AND (Inventory.ResponsiblePerson IN (SELECT LTRIM(Value) FROM F_SplitList(''', @responsiblePersons, ''',', ''','')))')

if @responsiblePersons2 is not null
    set @Query = concat(@Query, ' AND (Inventory.ResponsiblePerson2 IN (SELECT LTRIM(Value) FROM F_SplitList(''', @responsiblePersons2, ''',', ''','')))')

if @bulkClientCodes is not null
    set @Query = concat(@Query, ' AND (List.ClientCode IN (SELECT LTRIM(Value) FROM F_SplitList(''', @bulkClientCodes, ''',', ''','')))')
...
if @skipCount is not null 
    set @Query = concat(@Query, ' order by ', ISNULL(@sorting, 'Inventory.Id'), ' offset @skipCount rows fetch next @maxResultCount rows only')
