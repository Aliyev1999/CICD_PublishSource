CREATE OR ALTER procedure [dbo].[SP_WPM_GetDailyOperations](
    @firm smallint,
...
isnull(
    ...
    1
            ...
            dbo.F_WPM_IsAttachmentExists(TicketAction.Id)                                                                   AS IsAttachmentExists,
			    (SELECT STRING_AGG(j.[value], '', '') FROM OPENJSON(TicketAction.ActionParams, ''$.answers'') j)            AS Answers
    ...
    if @status is not null
        set @Query = concat(@Query,
                            case
                                when @status = 1
                                    then ' and (Ticket.FinalizedDate is not null and Ticket.StopReasonId is null) '
