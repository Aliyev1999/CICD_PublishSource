CREATE OR ALTER PROCEDURE [dbo].[SP_MGM_GetPhotoGalleryData] @userId BIGINT,
                                                    @begin datetime2,
                                                    @end datetime2,
                                                    @takeCount int,
                                                    @skipCount int
...
BEGIN
    ...
    with Data as (
        ...
        where Header.CreatedDate between @begin and @end
          and Files.Type = 3 and  actions.ActionType=1
		...
		select 
		Data.Url as PhotoUrl,
		Data.Id,
		Data.SourceType,
		Client.Code as ClientCode,
		Client.Name as ClientName,
		CASE
		    when Client.Latitude = 0 or Client.Latitude is null or OperationLatitude = 0 or OperationLatitude is null then null
		    when geography::Point(Client.Latitude, Client.Longitude, 4326).STDistance(geography::Point(OperationLatitude, OperationLongitude, 4326)) <= 300 THEN CAST(1 AS TINYINT)
		    when geography::Point(Client.Latitude, Client.Longitude, 4326).STDistance(geography::Point(OperationLatitude, OperationLongitude, 4326)) > 300 THEN CAST(2 AS TINYINT)
		    ELSE 0
		END as LocationType,
		Users.Id as UserId,
		...
		from Data
           left  join MD_Client Client with (nolock) on Client.TigerId = Data.ClientId and Client.Firm = Data.Firm --and Client.IsDeleted = 0
            ...
            left join (select distinct  SourceType, ReferenceId, CreatorUserId
			 ...
			 left join (
				select ReferenceId, SourceType,
					   sum(case when Status = 1 then 1 else 0 end) as LikedCount,
					   sum(case when Status = 0 then 1 else 0 end) as DislikedCount
				from (
					select distinct ReferenceId, SourceType, Status, CreatorUserId
					from MD_PhotoLike with (nolock)
				) UniqueReactions
				group by ReferenceId, SourceType
			) Reaction
    on Reaction.ReferenceId = Data.Id and Reaction.SourceType = Data.SourceType

             left join (select ReferenceId, SourceType, count(Id) as Count from MD_PhotoComment with (nolock) group by ReferenceId, SourceType) Comment
