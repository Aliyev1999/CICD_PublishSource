ALTER procedure [dbo].[SP_IM_GetAssetCountPerBindingForApp](
    @firm smallint,
    @userId bigint
)
AS
BEGIN
    ...
    with AssetCount as (select asset.Id               as AssetId,
                        ...
                        from IM_Asset asset with (nolock)
                                 ...
                                 left join MD_Salesman slsman with (nolock)
                                           on cast(slsman.Code as nvarchar(200)) collate SQL_Latin1_General_CP1_CI_AS = asset.BindingReference and
                                              slsman.Firm = asset.Firm and asset.BindingType = 2
                                 left join MD_Division division with (nolock)
                                           on cast(division.Nr as nvarchar(50)) = asset.BindingReference and division.Firm = asset.Firm and
                                              asset.BindingType = 3
                                 ...
                                 left join MD_Client client with (nolock)
                                           on cast(client.TigerId as nvarchar(50)) = asset.BindingReference and client.Firm = asset.Firm and
                                              asset.BindingType = 6
    ...
    from AssetCount
    group by BindingType, BindingReference, BindingReferenceName
