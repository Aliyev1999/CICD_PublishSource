CREATE OR ALTER PROCEDURE [dbo].[SP_CHL_CalculateComplexKpiMasterReport](@clientCodeOrName NVARCHAR(50) NULL,
...
BEGIN
    ...
    SET @Query =
            '
            WITH Structure AS (
            SELECT * FROM [dbo].[F_CHL_GetComplexStructureFullTransposed]() Structure
    ...
    SET @Query = CONCAT(@Query, '),
					...
					distinct
					Structure.*,
					Salesmans.Code OfficialName,
					Salesmans.Name FinCode,
					Points.UserPoint,
					Points.MaxPoint,
					Points.Ratio
					...
					FROM Structure INNER JOIN MD_Salesman Salesmans with (nolock)
                          ...
                          INNER JOIN (SELECT datepart(MONTH, Response.SavedDate) Month,
                                             ...
                                             ISNULL(CAST(0 AS FLOAT), 0) MaxPoint,
                                             ISNULL(round(iif(CAST(0 AS FLOAT)=0, 0, CAST(UserPoints.Point AS FLOAT) / CAST(0 AS FLOAT)) * 100, 2), 0) Ratio
