...
DECLARE @Query NVARCHAR(MAX);
    ...
    SET @Query =
            ...
            WHERE	 Structure.FirmNr = @firm
				...
				'
    IF @salesmanTigerId			  IS NOT NULL SET @Query = CONCAT(@Query, ' AND Structure.PersonId = @salesmanTigerId')
	...
	SET @Query = CONCAT(@Query, ')
						...
						SELECT
							...
							ISNULL(round(iif(CAST(MaxPoint.Point AS FLOAT) = 0, 0,
								CAST(UserPoint.Point AS FLOAT) / 100) * 100,
								2), 0)					  
								as  KPI
						...
						AND YEAR(Response.SavedDate)=Structure.Year AND Response.Firm = Structure.FirmNr AND cast(Response.SavedDate as date) BETWEEN cast(@savedDateStart as date) and cast(@savedDateEnd as date) 
						left JOIN CHL_UserResponsePoint UserPoint on UserPoint.ReportId = Response.Id and UserPoint.PointType=1
						JOIN CHL_Survey Survey ON Survey.Id = Response.SurveyId AND Survey.Firm = Response.Firm AND Survey.RepresentativeKPI = 1 AND Survey.IsDeleted = 0 and Survey.SurveyContentType = Structure.PackageType
