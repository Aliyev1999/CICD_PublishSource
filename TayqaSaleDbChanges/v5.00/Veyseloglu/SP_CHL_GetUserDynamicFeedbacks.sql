CREATE OR ALTER procedure [dbo].[SP_CHL_GetUserDynamicFeedbacks] @firm smallint, @userId bigint, @feedbackId bigint, @beginDate datetime, @endDate datetime
as
begin
    ...
    from CHL_UserSurveyResponse Response with (nolock)
             ...
             join MD_Client Client with (nolock) on Response.ClientId = Client.TigerId and Client.Firm = @firm
             ...
             inner join CHL_UserDynamicSurveyResponseDetail Detail on Response.Id = Detail.UserSurveyResponseId
             left join CHL_UserDynamicSurveyResponseDetailReason Reason on Detail.Id = Reason.UserDynamicSurveyResponseDetailId
    where Response.Firm = @firm
      and cast(Response.CreatedDate as date) between cast(@beginDate as date) and cast(@endDate as date)
      and (
          (@feedbackId is not null and  Response.Id = @feedbackId) OR
          (@feedbackId is null and  Response.UserId = @userId)
      )
    and Survey.Type = 4
