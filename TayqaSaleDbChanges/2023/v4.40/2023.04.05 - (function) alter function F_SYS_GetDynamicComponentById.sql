...
GO
ALTER function [dbo].[F_SYS_GetDynamicComponentById](@componentId int, @screenId int )
    Returns Table
        AS
        RETURN
            (
                ...
                from SYS_DynamicComponent d
                          join  SYS_MobileScreenCreateEditFields m on m.AdditionalExpression= d.Id
                         join SYS_DynamicComponentDisplayColumns c
                              on d.Id = c.ComponentId
                        where  d.Id=@componentId and m.MobileScreenId=@screenId
                group by d.Id,
                         d.Name,
                         d.TableName,
                         d.IsActive,
