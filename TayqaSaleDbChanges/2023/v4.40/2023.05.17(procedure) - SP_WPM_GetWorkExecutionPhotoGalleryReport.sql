CREATE PROCEDURE [dbo].[SP_WPM_GetWorkExecutionPhotoGalleryReport]
	@userId INT,
	@beginDate DATE,
	@endDate DATE,
	@firm SMALLINT
...
from (select Tasks.Id as TaskId, Image.SecureUrl, 1 as Type
      ...
      where Ticket.Firm = @firm) Images
		 ...
		 join MD_StopReason GroupReason with(nolock) on  GroupReason.Id=Tasks.GroupReason
		 join MD_StopReason CategoryReason with(nolock) on  CategoryReason.Id=Tasks.CategoryReason
		 join MD_StopReason SubcategoryReason with(nolock) on  SubcategoryReason.Id=Tasks.SubcategoryReason
where Tasks.Firm = @firm
  and (cast(Tasks.CreatedDate as date) between @beginDate and @endDate)
END