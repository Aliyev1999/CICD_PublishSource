Create procedure SP_IM_GetInventortyRepairDemands       (@inventoryId int null,  
                                                         @firm smallint null,  
...
begin  
    ...
    declare @sql nvarchar(max) =  
        ...
        with Data as  (select    
                             ...
                             CASE WHEN consumption.Id IS NOT NULL THEN cast(1 as bit) ELSE cast(0 as bit) END	    AS HasConsumptions,    
							 ...
							 iif(ActiveTask.DemandId is not null,cast(1 as bit),cast(0 as bit))						As HasActiveTask
							   ...
							   left join (select distinct DemandId from IM_RepairTask  where Status=3) t on t.DemandId=Demands.Id
							   left join (select distinct DemandId from IM_RepairTask  where Status<>3) ActiveTask on ActiveTask.DemandId=Demands.Id  
    ...
    IF @applyDateRangeFilter IS NOT NULL AND @applyDateRangeFilter = 1  
        SET @sql = CONCAT(@sql,  
                          ' AND (',  
                          '(@searchForDemandCreationTime = 1 AND CAST(Demands.CreationTime AS DATE) BETWEEN CAST(@startDate AS DATE) AND CAST(DATEADD(DAY, -1, @endDate) AS DATE))',  
                          ' OR ',  
                          '(@searchForDemandCreationTime = 0 AND CAST(Demands.ConfirmationTime AS DATE) BETWEEN CAST(@startDate AS DATE) AND CAST(DATEADD(DAY, -1, @endDate) AS DATE))',  
                          ')')  
