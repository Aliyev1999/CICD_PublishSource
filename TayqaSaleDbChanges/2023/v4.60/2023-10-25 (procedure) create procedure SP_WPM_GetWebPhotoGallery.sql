CREATE  procedure [dbo].[SP_WPM_GetWebPhotoGallery] @withTask bit = null,
...
FROM CHL_Attachment Files with (nolock)
         ...
         LEFT JOIN (SELECT DISTINCT Mapping.AttachmentId,
                    ...
                    FROM WPM_PhotoTaskMapping Mapping with (nolock)
                             ...
                             LEFT JOIN WPM_TaskTicket TT with (nolock) ON Mapping.TaskId = TT.TaskId and Mapping.SourceType = 1) TaskCount
...
WHERE Files.Type = 3
  and Files.CreatedDate between @minDate and @maxDate ')
...
FROM WPM_Attachment Files with (nolock)
         ...
         LEFT JOIN (SELECT DISTINCT Mapping.AttachmentId,
                    ...
                    FROM WPM_PhotoTaskMapping Mapping with (nolock)
                             ...
                             join WPM_Task Task with (nolock) on Mapping.TaskId = TAsk.Id
                             LEFT JOIN WPM_TaskTicket TT with (nolock) ON Mapping.TaskId = TT.TaskId and Mapping.SourceType = 2) TaskCount
...
from OP_FileUploadLog Files with (nolock)
         ...
         LEFT JOIN (SELECT DISTINCT Mapping.AttachmentId,
                    ...
                    FROM WPM_PhotoTaskMapping Mapping with (nolock)
                             ...
                             join WPM_Task Task with (nolock) on Mapping.TaskId = TAsk.Id
                             LEFT JOIN WPM_TaskTicket TT with (nolock) ON Mapping.TaskId = TT.TaskId and Mapping.SourceType = 5) TaskCount
...
where FileCreatedDate is not null
  and Files.FileCreatedDate between @minDate and @maxDate
...
from IM_InventoryStateHistoryImage Files with (nolock)
         ...
         LEFT JOIN (SELECT DISTINCT Mapping.AttachmentId,
                    ...
                    FROM WPM_PhotoTaskMapping Mapping with (nolock)
                             ...
                             join WPM_Task Task with (nolock) on Mapping.TaskId = TAsk.Id
                             LEFT JOIN WPM_TaskTicket TT with (nolock) ON Mapping.TaskId = TT.TaskId and Mapping.SourceType = 4) TaskCount
...
where History.CreatedDate between @minDate and @maxDate ')
    if @taskStatus is not null and @withTask = 1
        set @Query = concat(@Query, case @taskStatus
                                        when 1 then ' and (TaskCount.IsStarted=0)'
                                        when 2 then ' and (TaskCount.IsCompleted=0)'
...
from OP_FileUploadLog Files with (nolock)
         ...
         LEFT JOIN (SELECT DISTINCT Mapping.AttachmentId,
                    ...
                    FROM WPM_PhotoTaskMapping Mapping with (nolock)
                             ...
                             join WPM_Task Task with (nolock) on Mapping.TaskId = TAsk.Id
                             LEFT JOIN WPM_TaskTicket TT with (nolock) ON Mapping.TaskId = TT.TaskId and Mapping.SourceType = 3) TaskCount
                   on TaskCount.AttachmentId = Files.Id and TaskCount.Firm = Files.Firm
         Left join (SELECT DISTINCT Mapping.AttachmentId,
                                    Mapping.Firm,
                                    COUNT(distinct Mapping.TaskId) as TaskCount
...
where Files.FilePath is not null
  and Visit.CreatedDate between @minDate and @maxDate
          and Files.FilePath != ''''')
    if @taskStatus is not null and @withTask = 1
        set @Query = concat(@Query, case @taskStatus
                                        when 1 then ' and (TaskCount.IsStarted=0)'
