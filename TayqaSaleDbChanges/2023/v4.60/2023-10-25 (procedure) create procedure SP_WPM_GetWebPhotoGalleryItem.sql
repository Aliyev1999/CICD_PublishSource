CREATE PROCEDURE [dbo].[SP_WPM_GetWebPhotoGalleryItem]
    @referenceId INT,
    @sourceType TINYINT
AS
BEGIN
    ...
    BEGIN
        SELECT 
            ...
            fn.Answers AS Detail4,
            fn.Reasons AS Detail5,
        ...
        FROM CHL_Attachment Files with (nolock)
						   ...
						   left join CHL_UserSurveyResponseDetailAttachmentReason AtachReas with(nolock) on AtachReas.UserSurveyResponseDetailId=Files.ReferenceId and AtachReas.AttachmentId=Files.Id 
                  ...
                  WHERE Files.Type = 3 and Files.Id=@referenceId
    END
    ...
    BEGIN
        ...
        FROM WPM_Attachment Files with (nolock)
                  ...
                  WHERE TaskAction.ActionType = 1
                    AND Files.Type = 3 AND Files.Id = @ReferenceId
    END
    ...
    BEGIN
       select Files.Id								as Id,
                  ...
                  where FileCreatedDate is not null
                    and Files.ContentType = 1 and Files.Id=@referenceId
	...
	begin 
	 select  Files.Id	   					  	    as Id,
                 ...
                 Left join (SELECT DISTINCT Mapping.AttachmentId,
                    ...
                    GROUP BY Mapping.AttachmentId, Mapping.Firm) TaskCountMain
                   on TaskCountMain.AttachmentId = Files.Id and TaskCountMain.Firm = Files.Firm
        where Files.FilePath is not null  and Files.FilePath != '' and Files.Id=@referenceId
	end
