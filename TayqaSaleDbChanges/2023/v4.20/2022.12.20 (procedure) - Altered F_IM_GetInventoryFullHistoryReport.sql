drop function F_IM_GetInventoryFullHistoryReport
go
create FUNCTION [dbo].[F_IM_GetInventoryFullHistoryReport](@inventoryId int,
    ...
    begin
        ...
        if CHARINDEX('WarehouseTransfer', @sourceTypes) > 0 or @sourceTypes is null
            BEGIN
                ...
                from IM_Inventory inventory with (nolock)
                         join IM_WarehouseTransferLineInventory wInventory with (nolock)
                              on inventory.ID = wInventory.InventoryId
                         join IM_WarehouseTransfer wtransfer with (nolock)
                              on wtransfer.Id = wInventory.WarehouseTransferId and wtransfer.Status = 0
                         join MD_Warehouse towhouse with (nolock)
                              on towhouse.Nr = wtransfer.ToWarehouse and towhouse.Firm = wtransfer.Firm
                         join MD_Warehouse fromwhouse with (nolock)
                              on fromwhouse.Nr = wtransfer.FromWarehouse and fromwhouse.Firm = wtransfer.Firm
