ALTER procedure [dbo].[SP_WPM_GetWorkExecutionDetailsReport] @startDate datetime, @endDate datetime, @searchForCreationDate bit, @Firm smallint, @UserId int,
...
from WPM_UserTask TaskUsers with (nolock)
         ...
         join (select TaskId, count(Id) as TaskActionCount from WPM_TaskAction group by TaskId) TaskActions on TaskActions.TaskId = Tasks.Id
