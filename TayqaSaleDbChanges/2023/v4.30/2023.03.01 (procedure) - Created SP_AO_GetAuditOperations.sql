CREATE PROCEDURE [dbo].[SP_AO_GetAuditOperations] @firm smallint null,
...
-- Date: 01.03.2022 
-- Query: returns the result of Service Portal report for audit operations
...
from AO_AuditOperation operation with (nolock)
         ...
         join AbpUsers controlledUser with (nolock)
              on controlledUser.Id = operation.CreatedUserId and controlledUser.IsActive = 1 and controlledUser.IsDeleted = 0
...
if @isActDate = 1 set @WhereFilter =  '
 ...
 if @uId is not null set @WhereFilter =  '
 and (operation.UId = @uId)  '
...
if @controlledUserIds is not null set @Query = concat(@Query, ' and (operation.CreatedUserId in (select ltrim(Value) from F_SplitList(''', @controlledUserIds, ''',', ''',''))) ')
