CREATE function [dbo].[F_OP_GetThirdPartyDocuments]
(
	@userId int,
	@beginDate datetime,
	@endDate datetime,
...
from OP_ThirdPartyIncomingLog ILog with (nolock)
         ...
         left join (select Id, sum(Amount*Price) as Amount
		 ...
		 left join (select Id, sum(Amount) as Amount
                    from OP_ThirdPartyIncomingLogCommonLineExtension with (nolock)
                    group by Id) Line2 on Ilog.Id = line.Id
		 left join (select Id, sum(Amount) as Amount
...
from OP_ThirdPartyIncomingLog ILog with (nolock)
		 ...
		 left join (select Id, sum(Quantity ) as Amount
                    from OP_ThirdPartyIncomingLogWarehouseOperationLineExtension with (nolock)
                    group by Id) LineAmount on Ilog.Id = LineAmount.Id
		 left join OP_ThirdPartyRequestQueue queue ON queue.Id = Ilog.Id
