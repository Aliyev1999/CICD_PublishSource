ALTER procedure [dbo].[SP_CHL_GetUserSurveyQuestions] @surveyId int, @clientId int
as
begin
    ...
    from CHL_SurveyQuestion sq with (nolock)
             ...
             left join CHL_AnswerQuestionRelation aqr with (nolock)
                       on aqr.ParentQuestionId = q.Id and aqr.AnswerId = a.Id and aqr.SurveyId = s.Id
             ...
             left join CHL_Reasons r with (nolock) on a.Id = r.AnswerId
    where S.Id = @surveyId
      and sq.Status = 0
end
