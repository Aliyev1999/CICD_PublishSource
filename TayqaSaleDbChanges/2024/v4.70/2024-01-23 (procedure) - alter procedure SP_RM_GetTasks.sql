    ...
    declare @UnAssignedTaskCount int = (select count(distinct TaskClients.Id)
		      ...
		      with Data as (select TaskClient.Id as ClientId,
                  ...
                  from WPM_UserTask UserTask with (nolock)
					       ...
					       join F_GetAllPermittedClient() PermittedClients on PermittedClients.UserId = UserTask.UserId and PermittedClients.Firm = Tasks.Firm
                           join WPM_TaskClient TaskClient with (nolock) on UserTask.TaskId = TaskClient.TaskId and TaskClient.ClientId = PermittedClients.ClientId 
                  ...
                  group by TaskClient.Id,TaskTickets.Id,UserTask.UserId)
