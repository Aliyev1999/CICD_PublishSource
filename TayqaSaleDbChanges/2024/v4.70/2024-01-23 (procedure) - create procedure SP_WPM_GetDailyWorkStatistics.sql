CREATE procedure [dbo].[SP_WPM_GetDailyWorkStatistics]
(
...
select cast(Ticket.UserId as bigint)                                                                                  as UserId,
       ...
       isnull(
               round(
                       iif(
                                       Client.Latitude = 0 or Client.Latitude is null OR Ticket.CreatedLatitude is null or
   ...
   if @users is not null 
	...
	if @taskStatus	is not null
		set @Query =concat(@Query,case @taskStatus
                                ...
                                when 3 then ' and (Ticket.IsCompleted=0 and Ticket.StopReasonId is null)'--Completed
								when 4 then ' and (Ticket.IsCompleted=0 and Ticket.StopReasonId is not null)'--Stopped
                                end)
