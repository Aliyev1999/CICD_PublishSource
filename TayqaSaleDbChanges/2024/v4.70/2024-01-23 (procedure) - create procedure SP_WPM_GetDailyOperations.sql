CREATE   procedure [dbo].[SP_WPM_GetDailyOperations](
    @firm smallint,
...
isnull(
    round(
        iif(
            Client.Latitude = 0 or Client.Latitude is null OR TicketAction.CreatedLatitude is null or TicketAction.CreatedLatitude is null,
            -1,
            geography::Point(Client.Latitude, Client.Longitude, 4326).STDistance(
                geography::Point(TicketAction.CreatedLatitude, TicketAction.CreatedLongitude, 4326)
            ) / 1000  -- Divide by 1000 to get distance in kilometers
        ), 
        2
    ), 
    0
...
isnull(
    round(
        iif(
            Client.Latitude = 0 or Client.Latitude is null OR TicketAction.CreatedLatitude is null or TicketAction.FinalizedLatitude is null,
            -1,
            geography::Point(Client.Latitude, Client.Longitude, 4326).STDistance(
