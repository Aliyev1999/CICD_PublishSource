...
BEGIN
    ...
    with RouteData as (select Route.TigerClientId,
                       ...
                       where Route.Date between cast(@startDate as date) and dateadd(day, -1, cast(@endDate as date))
                         and Route.Firm = @firm
                         and Route.Status = 0
         ...
         Visits as (
                ...
                where Visit.Firm = @firm and ClientId=@clientId
                  and cast(Visit.CreatedDate as date) between cast(@startDate as date) and dateadd(day, -1, cast(@endDate as date))
                ...
                WHERE Ticket.Firm = @firm and Task.Type=1
                  ...
                  AND CAST(Ticket.CreatedDate AS DATE) BETWEEN CAST(@startDate AS DATE) AND dateadd(day, -1, cast(@endDate as date)))
            ,
