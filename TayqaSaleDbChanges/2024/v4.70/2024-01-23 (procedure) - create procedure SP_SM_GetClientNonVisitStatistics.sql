CREATE procedure [dbo].[SP_SM_GetClientNonVisitStatistics]
(
	@startDate datetime,
	@endDate datetime,
...
declare @Query nvarchar(max) ='declare @Result table
					...
					ClientCode nvarchar(255),Date date);
...
if @clientCodeOrName is not null
    ...
    set @Query = concat(@Query, '
		...
		insert into @Result (UserFullName ,UserId ,ClientTigerId ,ClientName ,ClientCode ,Date   )
