alter function [dbo].[F_IM_GetInventoryFullHistoryReport](@inventoryId int,
    ...
    begin
        if CHARINDEX('TransferDemand', @sourceTypes) > 0 or @sourceTypes is null
            BEGIN
                ...
                SELECT transferDemand.Id							as OperationId,
					   ...
					   creatorUser.Name +' '+ creatorUser.Surname	as UserFullName,
        ...
        if CHARINDEX('WarehouseTransfer', @sourceTypes) > 0 or @sourceTypes is null
            ...
            END
        if CHARINDEX(',' + 'RepairDemand' + ',', ',' + @sourceTypes + ',') > 0
			OR CHARINDEX('RepairDemand' + ',', @sourceTypes) = 1
			OR CHARINDEX(',' + 'RepairDemand', @sourceTypes) = LEN(@sourceTypes) - LEN(REPLACE(@sourceTypes, ',', '')) + 1
			OR @sourceTypes is null
            Begin
                insert into @fullHistory
            ...
            End
		if CHARINDEX('RepairDemandInWarehouse', @sourceTypes) > 0 or @sourceTypes is null
			Begin
				insert into @fullHistory
				select d.Id									as OperationId,
					   d.InventoryId						as InventoryId,
					   ...
					   u.Name +' '+ u.Surname				as UserFullName,
					   cast(1 as tinyint)					as OperationType,
					   'RepairDemandInWarehouse'			as SourceType,
					   
					   CASE
                           WHEN (SELECT COUNT(*)
                                 FROM IM_WarehouseRepairAttachment
                                 WHERE IM_WarehouseRepairAttachment.ReferenceId = d.Id
                                   and Type = 1) > 0
                               THEN CONVERT(Bit, 1)
                           ELSE CONVERT(Bit, 0)
                           END						as HasImages

				from IM_WarehouseRepairDemand d with (nolock)
						join MD_Warehouse w with (nolock) on d.WarehouseNr = w.Nr and d.Firm = w.Firm
						left join AbpUsers u with (nolock) on d.CreatorUserId = u.Id
				where d.InventoryId = @inventoryId
					AND (@startDate Is NULL OR d.CreationTime >= @startDate)
					AND (@endDate Is NULL OR d.CreationTime <= @endDate)
