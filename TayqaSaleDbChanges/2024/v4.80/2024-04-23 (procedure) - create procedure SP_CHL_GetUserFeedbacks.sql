create procedure [dbo].[SP_CHL_GetUserFeedbacks] @firm smallint, @userId bigint, @beginDate datetime, @endDate datetime
as
begin
...
from CHL_UserSurveyResponse Response with (nolock)
         ...
         left join (select json_value(ActionParams, '$.surveyId') as SurveyId, TicketAction.Id as TicketActionId
                    ...
                    where json_value(ActionParams, '$.surveyId') is not null
                      and json_value(ActionParams, '$.surveyId') <> 0) TicketAction on Response.SurveyId = TicketAction.TicketActionId
