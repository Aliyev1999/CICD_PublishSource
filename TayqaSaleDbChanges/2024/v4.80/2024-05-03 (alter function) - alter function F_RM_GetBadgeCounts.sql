                ...
                SELECT (SELECT count(*)
                       ...
                       (SELECT COUNT(*)
                        ...
                        WHERE AssignedUserId = @userId
                          AND Demand.Firm = @firm
                          AND task.Status = 5
                          AND task.CreationTime >= DATEADD(DAY, -30, GETDATE())) as WarehouseNotStartedTaskCount,
                       ...
                       (SELECT COUNT(*)
                        ...
                        WHERE AssignedUserId = @userId
                          AND Demand.Firm = @firm
                          AND task.Status IN (5, 6, 7, 12)
                          AND task.CreationTime >= DATEADD(DAY, -30, GETDATE())) as WarehouseRepairTaskCount,
                       ...
                       (SELECT COUNT(*)
                        ...
                        where Demand.CreatorUserId = @userId
                          AND Firm = @firm
                          AND task.Status IN (9)
                          AND task.CreationTime >= DATEADD(DAY, -30, GETDATE())) as WarehouseRepairExecutionCount,

                       (SELECT COUNT(*) 
					   FROM IM_InventoryDemand Demand with(nolock)
					   where Demand.CreatorUserId in (select UserId from F_GetPermittedUsers(@UserId))
					   and Demand.Firm=@firm
					   and Demand.DemandStatus in (0)
					   and Demand.CreationTime>=DATEADD(DAY, -30, GETDATE()))    as PendingInventoryRequestsCount
            )