ALTER proc [dbo].[SP_MIP_GetOrderManagementData] @Firm smallint = null,
...
--     Confirmed = 8
    ...
    --select Lines.Id, round(sum(Amount * Price) - sum(iif(DiscountAmount is null, 0,DiscountAmount )),4) as Amount
	...
	select Firm. Name                 as Firm,
           ...
           cast(case
				...
				when Glo.ImportResult is null and th.Step = 11 and Queue.Id is not null then 99
           ...
           --round(LineTotal. Amount,4)        as Amount,
           round(cast(JSON_VALUE(Erp.ImportResult, ''$.ERPDocInfo.GrossAmount'') as float),4)         as NetAmount,
           ...
           isnull(JSON_VALUE(erp.ImportResult, ''$.ERPDocInfo.FicheNo''),'''')            as DocNumber,
    ...
    from OP_IncomingLog ILog with (nolock)
			 ...
			 left join OP_ThirdPartyIncomingLog th on th.Id = ILog.Id 
			 and th.UserId = @CurrentUserId 

    where ILog. DocType = 0  and ILog. ProcessDate between @OrderBeginDate and @OrderEndDate 
	AND (
    @CurrentUserId <> 54170  
    OR (ILog.SalesmanRef IN 
	(34268,36538,37850,38555,58480,42302,45708)))
  '
