ALTER function [dbo].[F_OP_GetThirdPartyDocuments](
    ...
    returns table
        as return(select ILog.Id                                                as Id,
                         cast(ILog.DocType as smallint)                         as Type,
                         ILog.DocNumber                                         as DocNumber,
                         round(isnull(Line.Amount, 0), 2)                       as Amount,
                         round(isnull(Line.Quantity, 0), 2)                     as Quantity,
                         ILog.UserId                                            as CreatorUserId,
                         case
                             when ILog.DocType in (1, 2) and ILog.Specode = 'Z12' then N'Saf qaytarma'
                             when ILog.DocType in (1, 2) and ILog.Specode = 'Z13' then N'Silinme'
                             when ILog.DocType in (1, 2) and ILog.Specode = 'Z14' then N'Müşteri Transfer İade'
                             when ILog.DocType in (1, 2) and ILog.Specode = 'Z19' then N'Sicak Mal Talebi Iade'
                             when ILog.DocType in (1, 2) and ILog.Specode = 'Z37' then N'SAF-keyfiyyət qaytarması'
                  ...
                  from OP_ThirdPartyIncomingLog ILog with (nolock)
                           ...
                           join MD_Client Client with (nolock)
                                on ILog.ClientId = Client.TigerId and ILog.Firm = Client.Firm
                           join AbpUsers Users with (nolock) on Users.Id = ILog.UserId
                           join SPEC_ReturnClientForConformation Spec with (nolock) on ILog.ClientId = Spec.ClientCode
                      ...
                      --join F_UIM_GetOrganizationTreeUsers(@userId) ou on ou.UserId = Users.Id
                           ...
                           left join MD_Division Division with (nolock)
                                     on Division.Nr = ILog.Division and ILog.Firm = Division.Firm
                           ...
                           left join (select distinct Id from OP_ThirdPartyCommonLineResultLog) lineResult
                                     on ILog.Id = lineResult.Id
                           left join (select Extension.Id,
                                             sum(iif(ResultLog.Amount is not null, ResultLog.Amount, Extension.Amount) *
                                                 Price) - Extension.DiscountAmount                                      as Amount,
                                             sum(iif(ResultLog.Amount is not null, ResultLog.Amount, Extension.Amount)) as Quantity
                                      from OP_ThirdPartyIncomingLogCommonLineExtension Extension with (nolock)
                                               left join OP_ThirdPartyCommonLineResultLog ResultLog
                                                         on Extension.Id = ResultLog.Id and
                                                            ResultLog.ItemId = Extension.ItemId
                           ...
                           left join OP_ThirdPartyIncomingLogCommonExtension WarehouseId with (nolock)
                                     on WarehouseId.Id = ILog.Id
                           left join MD_Warehouse Warehouse with (nolock)
                                     on WarehouseId.WhouseNr = Warehouse.Nr and Warehouse.Firm = ILog.Firm
                  where Ilog.DocType in (0, 1, 2, 3, 4)
                    and ILog.Firm = 10
                    and @firm = 10
                    and cast(ILog.RegisteredDate as date) between cast(@beginDate as date) and cast(@endDate as date)
                    and ILog.OperationType = 'Confirm1'
                    and Spec.HeadDepartmentCode = (select Code collate SQL_Latin1_General_CP1_CI_AS from AbpUsers with (nolock) where Id = @userId)
        ...
        --       cast(ILog.DocType as smallint)                              as Type,
        --       ILog.DocNumber                                              as DocNumber,
        --       round(coalesce(lineResultAmount.Amount, Line.Amount, 0), 2) as Amount,
        --       round(isnull(0, 0), 2)                                      as Quantity,
        --       ILog.UserId                                                 as CreatorUserId,
        ...
        --         left join OP_ThirdPartyRequestQueue RequestQueue with (nolock) on RequestQueue.Id = ILog.Id
        --         left join (select distinct Id from OP_ThirdPartyCashResultLog) lineResult
        --                   on ILog.Id = lineResult.Id
        --         left join (select Id, sum(Amount) as Amount
        --                    from OP_ThirdPartyCashResultLog
        --                    group by Id) lineResultAmount on ILog.Id = lineResultAmount.Id
        --         left join OP_ThirdPartyIncomingLogCommonExtension WarehouseId with (nolock)
        --                   on WarehouseId.Id = ILog.Id
        --         left join MD_Warehouse Warehouse with (nolock)
        ...
        --  and ILog.Firm = @firm
        --  and ILog.ProcessDate between cast(@beginDate as date) and cast(@endDate as date)
        ...
        --                round(isnull(LineAmount.Amount,0), 2)                  as Quantity,
        --                ILog.UserId                                            as CreatorUserId,
        --                Users.UserName                                         as CreatorUserName,
        --                concat(Users.Name, ' ', Users.Surname)                 as CreatorUserFullName,
        --                Salesman.Code                                          as SalesmanCode,
        ...
        --         left join OP_ThirdPartyResultLog Feedback with (nolock) on Feedback.Id = ILog.Id
        --         left join (select Extension.Id,
        --                           sum(iif(ResultLog.Quantity is not null, ResultLog.Quantity,
        --                                   Extension.Quantity)) as Amount
        ...
        --         left join OP_ThirdPartyRequestQueue queue ON queue.Id = Ilog.Id
        --         left join (select distinct Id from OP_ThirdPartyWarehouseOperationLineResultLog) lineResult
        --                   on ILog.Id = lineResult.Id
        --         left join (select Id, sum(Quantity) as Amount
        --                    from OP_ThirdPartyWarehouseOperationLineResultLog
        --                    group by Id) lineResultAmount on ILog.Id = lineResultAmount.Id
        --         left join OP_ThirdPartyRequestQueue RequestQueue with (nolock) on RequestQueue.Id = ILog.Id
        --         left join AbpUsers FeedbackUsers with (nolock) on FeedbackUsers.Id = Feedback.FeedbackUserId
        --         left join OP_ThirdPartyIncomingLogWarehouseOperationExtension WarehouseId with (nolock)
        --                   on WarehouseId.Id = ILog.Id
        --         left join OP_ThirdPartyIncomingLogWarehouseOperationLineExtension Line with (nolock)
        --                   on Line.Id = ILog.Id
        --         left join MD_Warehouse InWarehouse with (nolock)
        --                   on WarehouseId.WarehouseIn = InWarehouse.Nr and InWarehouse.Firm = ILog.Firm
        --         left join MD_Warehouse OutWarehouse with (nolock)
        --                   on WarehouseId.WarehouseOut = OutWarehouse.Nr and OutWarehouse.Firm = ILog.Firm
