...
select inventory.Id          as Id,
       ...
       ClientTigerId         as ClientTigerId,
...
from IM_Inventory inventory with (nolock)
         ...
         join MD_Item item with (nolock) on item.TigerId = inventory.TigerId and item.IsDeleted = 0
		 left join IM_StaticContent content with (nolock) on content.Id=inventory.StateId and content.Type=0

where inventory.Firm=@Firm 
	    and ClientTigerId=@ClientTigerId
