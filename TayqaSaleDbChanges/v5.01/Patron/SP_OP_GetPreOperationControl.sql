...
--if @docType in (2, 4, 5)
--    begin
--        select N'Icazə verilmir'                                                                                      as [Key],
--               N'SA kodlu mustərilərə sifariş, qaytarma fakturası və kassa mədaxil əməliyyatı etməyə icazə verilmir.' as Value
--        from MD_Client
...
--    end
    ...
    if @docType = 4 -- yalniz satis fakturasi
        begin
            DECLARE @LastSaleClient INT
            DECLARE @LastSaleDate DATE

            SELECT TOP 1 @LastSaleClient = ClientId,
                         @LastSaleDate = CAST(DocCreatedTime AS DATE)
            FROM OP_IncomingLog
            WHERE UserId = @userId
              AND DocType = @docType
            ORDER BY DocCreatedTime DESC

            IF @LastSaleClient IS NOT NULL
                BEGIN
                    IF NOT EXISTS (SELECT 1
                                   FROM OP_ClientVisitLog
                                   WHERE CreatedUserId = @userId
                                     AND ClientId = @LastSaleClient
                                     AND CAST(CreatedDate AS DATE) >= @LastSaleDate)
                        BEGIN
                            SELECT N'İcazə verilmir'                                     AS [Key],
                                   N'Bir öncəki müştəridə ziyarət əməliyyatı olunmayıb.' AS Value,
                                   null                                                  as LinkType,
                                   null                                                  as Link
                            WHERE @userId IN (SELECT UserId FROM MD_PermittedFirm WHERE Firm = 2)
                        END
                END
        end
end