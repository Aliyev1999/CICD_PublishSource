...
begin
    ...
    --    set @MonthHalf = 2
    declare @MandatoryTaskCount int = 0

    if
        @userActionType <= 32
        set @MandatoryTaskCount = (select count(Task.Id)
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------
    declare @MandatoryRouteTaskCount int = 0
    if
        @userActionType <= 32
        set @MandatoryRouteTaskCount = (select count(Task.Id)
                                        from WPM_Task Task with (nolock)
                                                 join WPM_UserTask Users with (nolock) on Users.TaskId = Task.Id
                                                 join WPM_TaskSchedule Schedule with (nolock) on Schedule.TaskId = Task.Id
                                                 join MD_Route r
