    ...
    begin
        ...
        with Reasons as ( select  distinct Detail.Id                                                                     as DetailId,
      		   ...
      		   select distinct Detail.Id                                        as DetailId,
                                ...
                                coalesce(NewReason.Value, Reason.Name , NewReason.Value, 'NoReason') collate SQL_Latin1_General_CP1_CI_AS as Reason
...
union all
    ...
    where Detail.UserSurveyResponseId = @ReportId
		union all
   select   Detail.Id                                                              as QuestionId,
                concat(Detail.InvoiceNo , ' ' , N'nömrəli faktura' )               as QuestionName,
               Detail.InvoiceNo collate SQL_Latin1_General_CP1_CI_AS               as QuestionCode,
               Response.RegisteredDate          as ResponseRegisteredDate,
               concat(Detail.InvoiceNo , ' ' , N'nömrəli faktura') collate SQL_Latin1_General_CP1_CI_AS     as QuestionDescription,
               ''                                                                  as Specode1,
               ''                                                                  as Specode2,
               ''                                                                  as Specode3,
               3                                                                   as QuestionType,
               ...
               ''                                    as MaxPoint
								from CHL_UserSurveyResponse Response with (nolock)
                         join GetLastInvoiceNoForAllClientUser() Detail  on Detail.ClientId = Response.ClientId and Detail.UserId = Response.UserId 
						 and cast(Detail.RegisteredDate as date) = cast(Response.SavedDate as date)
						 left join CHL_UserDynamicSurveyResponseDetail det with (nolock) on det.QuestionCode = Detail.InvoiceNo
                         where Response.Id = @ReportId and det.Id is  null
        return
    end