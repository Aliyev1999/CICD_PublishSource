...
if @userActionType < 37  and  (@clientId not in (Select ClientId from MD_ClientGpsData) and @clientId  in (select TigerId from MD_Client where Longitude = 0) and @clientId not in (select TigerId from PreOperationcontrol_except_clients))
	begin
		select N'xeta' as [Key], N'Müştəri mövqeyi tapılmadı' as Value	
		return
	end 
