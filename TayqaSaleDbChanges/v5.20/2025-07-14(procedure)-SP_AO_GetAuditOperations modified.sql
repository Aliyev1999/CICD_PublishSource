CREATE OR ALTER PROCEDURE [dbo].[SP_AO_GetAuditOperations] @firm smallint = null,
...
select operation.Id                                              as Id,
	   ...
	   SalesmanHead.Name                                     as SalesPersonHeadName,
	   SalesmanHead.Code                                              as SalesPersonHeadCode,
	   ...
	   iif(avoidanceReason.Type=25 or avoidanceReason.Type=24 ,CancelledUser.UserName,null )															 as CancelledUser,
	   CAST(0 AS BIT)														 as HasTaskRequest
...
from AO_AuditOperation operation with (nolock)
		 ...
		 left join MD_Salesman Salesman with(nolock) on Salesman.TigerId=operation.SalesmanId
		 left join MD_Salesman SalesmanHead with(nolock) on SalesmanHead.TigerId=operation.SalesPersonHeadId
		 ...
		 left join AbpUsers CancelledUser with (nolock) 
                   on CancelledUser.Id = operation.LastModifierUserId and CancelledUser.IsActive = 1 and CancelledUser.IsDeleted = 0 and (avoidanceReason.Type  = 24 Or avoidanceReason.Type = 25)
 ...
 and (operation.ActDate between @startDate and @endDate)  '
    ...
    if @uId is not null set @Query = concat(@Query, ' and (operation.UId = @uId)  ')
    ...
    if @clientNameCodeGroupCode is not null
        set @Query = concat(@Query,
                            ' and (client.Code like ''%''+@clientNameCodeGroupCode+''%'' or client.Name like ''%''+@clientNameCodeGroupCode+''%''or Client.Edino like ''%''+@clientNameCodeGroupCode+''%'') ')
    if @reasonIds is not null
        set @Query =
                concat(@Query, ' and (reason.Id in (select ltrim(Value) from F_SplitList(''', @reasonIds, ''',',
                       ''',''))) ')
    if @controlledUserIds is not null
        set @Query = concat(@Query, ' and (operation.ControlUserId in (select ltrim(Value) from F_SplitList(''',
                            @controlledUserIds, ''',', ''',''))) ')
    if @debtCheckOperationStatus is not null
        set @Query = concat(@Query, ' and operation.OperationStatus =@debtCheckOperationStatus')
    ...
    if @closerUserIds is not null
        set @Query = concat(@Query, ' and (operation.LastModifierUserId in (select ltrim(Value) from F_SplitList(''',
                            @closerUserIds, ''',', ''','')))  ')
