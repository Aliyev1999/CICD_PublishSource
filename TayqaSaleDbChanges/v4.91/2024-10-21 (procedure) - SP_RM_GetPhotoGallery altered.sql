ALTER procedure dbo.SP_RM_GetPhotoGallery @beginDate datetime, @endDate datetime, @userId bigint, @firm smallint, @takeCount int,
                                              @skipCount int, @clientId int, @totalImageCount int out
AS
BEGIN
	set @totalImageCount = 1;
	declare @Result table
					(DateTime datetime, CreatorUserName nvarchar(255), CreatorUserFullName nvarchar(255), Photo varchar(max), SecureUrl varchar(max), 
					 ClientCode nvarchar(255), ClientId int, ClientName nvarchar(255), LikeStatus bit, StarCount tinyint, CommentedUserName nvarchar(255),
					 CommentedUserFullName nvarchar(255), CommentReason nvarchar(max), CommentText nvarchar(max), TaskId int, Type int, LikerDislikerUser nvarchar(255));
...
insert into @Result
    ...
    from (select Files.CreatedDate                                                          as DateTime,
          ...
          where Files.CreatedDate between @beginDate and @endDate
            ...
            and Users.Id = @userId
			and (@clientId is null or Clients.TigerId = @clientId)
		 ...
		 union all
          ...
          where Files.FileCreatedDate  between @beginDate  and @endDate  and Files.ContentType=1
            and Users.Id = @UserId
			and (@clientId is null or Client.TigerId = @clientId)) Result
