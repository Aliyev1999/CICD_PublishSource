drop function F_CHL_GetSurveyQuestionReportAnswersDetailedFeedback
GO
create FUNCTION [dbo].[F_CHL_GetSurveyQuestionReportAnswersDetailedFeedback](
    @beginDate DATETIME,
    @endDate DATETIME,
    ...
    begin
        ...
        with AllReasons as (select Response.Id                               as UserSurveyResponseId,
                                   ...
                                   case
                                       ...
                                       when NewReason.Type is null and
                                            (Detail.ReasonId is not null or Detail.ReasonValue is not null) then 1
                                       end                                   as ReasonType,
                                   case
                                       when coalesce(NewReason.Value collate SQL_Latin1_General_CP1_CI_AS,
                                                     Detail.ReasonValue) is not null then 0
                                       else
                            ...
                            from CHL_UserSurveyResponseDetail Detail with (nolock)
                                     join CHL_UserSurveyResponse Response with (nolock)
                                          on Response.Id = Detail.UserSurveyResponseId
                                     ...
                                     left join MD_StopReason OldContent with (nolock)
                                               on OldContent.Id = NewReason.ReasonId
...
-- case 1: @answersInSeparatedLines = 1 and @reasonsInSeparatedLines = 1 -> simple select
        if @answersInSeparatedLines = 1 and @reasonsInSeparatedLines = 1
            ...
            select UserSurveyResponseId,
                   QuestionId,
                   Answer                               as Answers,
                   iif(Reason = 'NoReason', '', Reason) as Reasons,
                   ReasonType,
                   AnswerId
            from @Data
...
-- case 2: @answersInSeparatedLines = 1 and @reasonsInSeparatedLines = 0 -> Reasons grouped
        if @answersInSeparatedLines = 1 and @reasonsInSeparatedLines = 0
            ...
            select UserSurveyResponseId,
                   QuestionId,
                   Answer                                  as Answers,
                   coalesce(string_agg(Reason, ' / '), '') as Reasons,
...
-- case 3: @answersInSeparatedLines = 0 and @reasonsInSeparatedLines = 0
        if @answersInSeparatedLines = 0 --and @reasonsInSeparatedLines = 0
            ...
            select UserSurveyResponseId,
                   ...
                   coalesce(nullif(string_agg(Reasons, ' | '), 'NoReason'), '') as Reasons,
                   string_agg(ReasonType, '|')                                  as ReasonType,
                   string_agg(AnswerId, ' | ')                                  as AnswerId
            from (select UserSurveyResponseId,
                         QuestionId,
                         Answer                                          as Answers,
                         coalesce(string_agg(Reason, ' / '), 'NoReason') as Reasons,
