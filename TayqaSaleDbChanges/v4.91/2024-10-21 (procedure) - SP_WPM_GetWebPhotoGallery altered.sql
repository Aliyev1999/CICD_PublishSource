...
WHERE Files.Type = 3
  and cast(Files.CreatedDate as date) between cast(@minDate as date) and cast(@maxDate as date) ')
    ...
    if @surveyNameOrCode is not null and @surveyNameOrCode != ''
        set @Query = concat(@Query, ' and (Survey.Name IN (SELECT LTRIM(Value) FROM F_SplitList(''', @SurveyNameOrCode, ''',',
                            ''',''))  or Survey.Code IN (SELECT LTRIM(Value) FROM F_SplitList(''', @SurveyNameOrCode, ''',',
                            ''',''))) ')
	...
	select distinct Files.Id                                       as Id,
                ...
                Survey.Name                                        as SourceRef,
                isnull(Stars.StarCount, 0)                         as StarCount,
                Details.QuestionName                               as SourceDetail,
                Response.ClientId                                  as ClientId,
                Response.UserId                                    as UserId,
                ''''                                               as SourceDetailMessage,
                Client.Edino                                       as ClientEdino,
			    concat(cast(Files.CreatedDate as nvarchar(max)),''_'',Survey.Name,''_'',Details.QuestionCode ,''_'',Details.QuestionName,''_'',Details.AnswerValue) as Label
FROM CHL_Attachment Files with (nolock)
    ...
    set @Query = concat(@Query, '
         ...
         left join CHL_UserDynamicSurveyResponseDetailReason AtachReas with (nolock)
                   on AtachReas.UserDynamicSurveyResponseDetailId = Files.ReferenceId --and AtachReas.AttachmentId = Files.Id
         LEFT JOIN MD_PhotoLike Likes with (nolock) on Likes.ReferenceId = Files.Id and Likes.SourceType = 1
         LEFT JOIN MD_PhotoStar Stars with (nolock) on files.Id = Stars.ReferenceId and Stars.SourceType = 1
         LEFT JOIN AbpUsers LikeUser with (nolock) on LikeUser.Id = Likes.CreatorUserId
         LEFT JOIN (SELECT DISTINCT Mapping.AttachmentId,
  ...
  and cast(Files.CreatedDate as date) between cast(@minDate as date) and cast(@maxDate as date) ')
    ...
    if @surveyNameOrCode is not null and @surveyNameOrCode != ''
        set @Query = concat(@Query, ' and (Survey.Name IN (SELECT LTRIM(Value) FROM F_SplitList(''', @SurveyNameOrCode, ''',',
                            ...
                            ''',''))) ')
    if @questionNameOrCode is not null and @questionNameOrCode != ''
        set @Query = concat(@Query, ' and (Details.QuestionName like ''%''+@questionNameOrCode+''%'' or Details.QuestionCode like ''%''+@questionNameOrCode+''%'') ')
...
WHERE TaskAction.ActionType = 1
  ...
  AND Files.Type = 3 ')
    ...
    if @taskName is not null and @taskName != '' set @Query = concat(@Query, ' and (Task.Name IN (SELECT LTRIM(Value) FROM F_SplitList(''', @taskName, ''',', ''','')))');
...
from OP_FileUploadLog Files with (nolock)
    ...
    set @Query = concat(@Query, '
         ...
         left join MD_StopReason Reasons with (nolock) on Reasons.Id = Visit.ReasonId and Reasons.Type=4
         LEFT JOIN (SELECT DISTINCT Mapping.AttachmentId,
                                    Mapping.Firm,
                                    StopReasonId,
                                    IsCompleted                      as IsCompleted,
