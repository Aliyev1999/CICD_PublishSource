...
BEGIN
 ...
 --   INSERT INTO #ClientKPIUniqueClientIds
	...
	INSERT INTO #ClientKPIUniqueClientIds
	SELECT DISTINCT SCM.ClientId
	    FROM MD_SalesmanClientMapping SCM
	             JOIN MD_Salesman S ON SCM.Firm = S.Firm AND SCM.SalesmanId = S.TigerId
	             JOIN MD_Client C ON SCM.Firm = C.Firm AND SCM.ClientId = C.TigerId
	             JOIN F_GetAllPermittedClient() PC ON PC.Firm = C.Firm AND PC.ClientId = C.TigerId
	             JOIN #ClientKPIPermittedUserIds PU ON PU.UserId = PC.UserId
	    WHERE C.Firm = @firm
	      AND (@salesManCodesStr IS NULL OR s.Code IN (SELECT Value FROM F_SplitList(@salesManCodesStr, ',')))
    ...
    DECLARE @sql NVARCHAR(MAX) =
        'SELECT USR.SavedDate,
            ...
            FROM MD_Client C
					 ...
					 JOIN UIM_UserTypeUserMapping UTUM ON UTUM.UserId = USR.UserId					 
	...
	IF @userTypes IS NOT NULL
		SET @sql = CONCAT(@sql, ' and (UTUM.UserTypeId IN (SELECT * FROM F_SplitList(@userTypes, '','')))')
    ...
    EXEC sp_executesql @sql,
         ...
         @questionGroupsStr NVARCHAR(30) , @questionType INT , @questionSpecCode NVARCHAR(30), @surveyContentType INT, @userTypes NVARCHAR(MAX)',
         ...
         @questionGroupsStr = @questionGroupsStr, @questionType = @questionType, @questionSpecCode = @questionSpecCode, @surveyContentType = @surveyContentType, @userTypes = @userTypes
END
