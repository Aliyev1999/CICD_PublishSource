            ...
            (
                WITH cte AS (
                    SELECT soldData.ItemId,
                           ...
                           ((soldData.Amount * iu.Convfact2) / iu.Convfact1) / (iuLnr.Convfact2 / iuLnr.Convfact1) AS AmountWithLessUnit
                    FROM (
                             ...
                             FROM OP_IncomingLog il WITH (NOLOCK)
								  ...
								  JOIN MD_ClientGroupData clnt on clnt.ClientId = il.ClientId and clnt.Firm = il.Firm and clnt.GroupType = @groupType
								  JOIN MD_ClientGroupData clnt2
								on clnt2.Firm = clnt.Firm and clnt2.GroupId = clnt.GroupId and clnt.GroupType = clnt2.GroupType
                             WHERE gl.ImportResult = 0
                               AND il.Firm = @firm
                               AND il.ProcessDate >= @startDate
                               AND il.ProcessDate <= @endDate
                               AND clnt2.ClientId = @clientId
                               AND il.DocType = @operationId
                               AND (clx.ItemId IN (SELECT * FROM F_SplitList(@itemIds, ',')))
                         ) as soldData
                             JOIN MD_ItemUnit iu WITH (NOLOCK) ON soldData.ItemUnitCode = iu.Code AND soldData.ItemId = iu.TigerItemId AND iu.Firm = soldData.Firm
                             JOIN MD_ItemUnit iuLnr WITH (NOLOCK) ON soldData.ItemId = iuLnr.TigerItemId AND iuLnr.Firm = soldData.Firm AND iuLnr.LineNr = 1
                )
