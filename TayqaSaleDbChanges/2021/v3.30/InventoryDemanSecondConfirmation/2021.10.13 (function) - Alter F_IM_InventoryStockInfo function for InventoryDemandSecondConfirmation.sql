/****** Object:  UserDefinedFunction [dbo].[F_IM_InventoryStockInfo]    Script Date: 13.10.2021 18:16:46 ******/
ALTER FUNCTION [dbo].[F_IM_InventoryStockInfo](@userId INT, @firm SMALLINT)

RETURNS @Result Table(TigerId int, 
			          Firm  smallint, 
				      WarehouseNr int, 
					  UserId int, 
					  ActualAmount int, 
...
SELECT 
@secondConfirmEnabled  = CASE WHEN Value='true' then 1 else 0 END
FROM
SYS_GlobalConfigParameter
WHERE 
Name='InventoryDemandSecondConfirmEnabled' AND
Status = 1;

INSERT INTO @Result SELECT *FROM  (SELECT INV.TigerId,
										  INV.Firm,
									      INV.WarehouseNr,
										  PW.UserId,
									      ...
									      (SELECT COUNT(*)
										   ...
										   WHERE Status = 1
											...
											(SELECT COUNT(*)
												...
												AND Reserved = 1
												AND (@secondConfirmEnabled=0 OR Step != 2)))                AS RealAmount --Step = 2 is rejected 
