ALTER FUNCTION [dbo].[F_MIP_GetSalesmanRouteStatisticsMaster](@startDate date, @endDate date, @inRoute BIT NULL,
@clientFilterEnabled bit, @latStart float, @latEnd float, @longStart float, @longEnd float, @firm smallint)
RETURNS TABLE
AS
RETURN (
	WITH Users AS (
     ...
     InvoiceCounts AS (
         ...
         WHERE UAGD.SendDate >= @startDate
           AND UAGD.SendDate <= @endDate
           AND (@inRoute IS NULL OR
                (@inRoute = IIF((SELECT COUNT(*)
                FROM MD_Route r WITH (NOLOCK)
                WHERE r.Date >= @startDate
