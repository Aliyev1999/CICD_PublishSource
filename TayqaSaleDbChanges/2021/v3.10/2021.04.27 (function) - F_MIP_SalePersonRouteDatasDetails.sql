CREATE FUNCTION F_MIP_SalePersonRouteDatasDetails(@userId int,@processDate date)
RETURNS TABLE
AS
RETURN(
...
FROM
    ...
    SELECT
        ...
        from F_GetPermittedClientForUser(@userId) PC
            ...
            LEFT JOIN (SELECT ISNULL(r.TigerClientId, l.ClientId) TigerId, l.GpsDate VisitDate,
            ...
            FROM MD_Route r
                FULL JOIN OP_UserActionGpsData l ON
                    ...
                    CAST(l.GpsDate AS Date)=r.Date
                JOIN MD_Client c ON ISNULL(r.Firm, l.Firm) = c.Firm AND ISNULL(r.TigerClientId, l.ClientId) = c.TigerId
            WHERE 
                ISNULL(r.Date, CAST(l.GpsDate AS Date))= @processDate AND
                ISNULL(r.UserId, l.UserId) = @userId) ST on ST.TigerId = PC.ClientId
) T)) X
where RN=1
)