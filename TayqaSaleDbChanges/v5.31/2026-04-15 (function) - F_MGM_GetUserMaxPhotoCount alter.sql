create or ALTER FUNCTION [dbo].[F_MGM_GetUserMaxPhotoCount](@firm smallint, @userId BIGINT, @begin DATETIME2, @end DATETIME2, @takeCount INT, @skipCount INT)
    ...
    with Data as (
        ...
        from CHL_Attachment Files with (nolock)
                 ...
                 join F_GetPermittedUsers(@userId) Permitted on Permitted.UserId = Header.UserId
        where Header.SavedDate between @begin and @end and Header.Firm=@firm
          and Files.Type = 3
