...
create or alter function [dbo].[FN_MGM_GetClientGeneralDataForUser](
    ...
    returns table as
        ...
        select Client.TigerId                           as Id,
               ...
               case 
                   when BannedClient.Status = 0 then ISNULL(BannedClient.Note, '')
                   else null
               end                                      as BlockReason
        from MD_Client Client with (nolock)
                 join F_GetPermittedClientForUserWithRegisteredDate(@UserId) pclient 
                      on pclient.ClientId = Client.TigerId and pclient.Firm = Client.Firm
                 left join PermittedUsersAgg permitteduser 
                      on permitteduser.ClientId = Client.TigerId and permitteduser.Firm = Client.Firm
                 left join ClientDebt 
                      on ClientDebt.TigerClientId = Client.TigerId and ClientDebt.Firm = Client.Firm
                 left join Md_Currency Currency with (nolock) 
                      on ClientDebt.CurrencyType = Currency.Type and ClientDebt.Firm = Currency.Firm
                 left join MD_BannedClient BannedClient with (nolock)
                      on BannedClient.ClientId = Client.TigerId and BannedClient.Firm = Client.Firm
