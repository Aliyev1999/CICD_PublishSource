Create or ALTER procedure [dbo].[SP_MGM_GetUserMainScreenPendingData] @firm Smallint, @userId bigint 
as
begin
    ...
    set @NoFeedackImageCountToday = nullif((select pending
                                            from FN_MGM_GetDashboardData( @firm,@UserId, @PhotoBeginDateToday, @PhotoEndDate)), 0)
    set @NoFeedackImageCountLast30Days = nullif((select pending
                                                 from FN_MGM_GetDashboardData( @firm,@UserId, dateadd(day, -30, @PhotoBeginDateToday), @PhotoEndDate)), 0)
