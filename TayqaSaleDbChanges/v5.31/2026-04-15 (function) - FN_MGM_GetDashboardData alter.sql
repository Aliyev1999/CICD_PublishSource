    ...
    BEGIN
        WITH MainData AS (SELECT Files.SecureUrl   AS Url,
                          ...
                          FROM CHL_Attachment Files with (nolock)
                                   JOIN CHL_UserSurveyResponseDetail Details with (nolock)
                                        ON Files.ReferenceId = Details.Id
                                   JOIN CHL_UserSurveyResponse Response with (nolock)
                                        ON Details.UserSurveyResponseId = Response.Id
                                   ...
                                   left join MD_PhotoComment comment with (nolock)
                                             ON comment.ReferenceId = Files.Id AND comment.SourceType = 1
                          WHERE Files.Type = 3
                          ...
                          FROM CHL_Attachment Files with (nolock)
                                   JOIN CHL_UserDynamicSurveyResponseDetail Details with (nolock)
                                        ON Files.ReferenceId = Details.Id
                                   JOIN CHL_UserSurveyResponse Response with (nolock)
                                        ON Details.UserSurveyResponseId = Response.Id
                                   JOIN F_GetPermittedUsers(@userId) ou ON ou.UserId = Response.UserId
                          ...
                          FROM WPM_Attachment Files with (nolock)
                                   JOIN WPM_TaskTicketAction TicketActions with (nolock)
                                        ON Files.ReferenceId = TicketActions.Id
                                   ...
                                   left join MD_PhotoComment comment with (nolock)
                                             ON comment.ReferenceId = Files.Id AND comment.SourceType = 2
                          WHERE Files.Type = 3
                          ...
                          FROM OP_FileUploadLog Files with (nolock)
                                   JOIN MD_Client Client with (nolock)
                                        ON Client.TigerId = Files.ClientId --AND Client.Firm = 9
                                   JOIN F_GetPermittedUsers(@userId) ou ON ou.UserId = Files.UploadedUserId
                                   LEFT JOIN MD_PhotoLike Likes with (nolock)
                                             ON Files.Id = Likes.ReferenceId AND Likes.SourceType = 5
                                   left join MD_PhotoComment comment with (nolock)
                                             ON comment.ReferenceId = Files.Id AND comment.SourceType = 5
                          ...
                          FROM OP_FileUploadLog Files with (nolock)
                                   JOIN F_GetPermittedUsers(@userId) ou ON ou.UserId = Files.UploadedUserId
                                   LEFT JOIN MD_PhotoLike Likes with (nolock)
                                             ON Files.Id = Likes.ReferenceId AND Files.ContentType = 2 AND
                                                Likes.SourceType = 3
                                   left join MD_PhotoComment comment with (nolock)
                                             ON comment.ReferenceId = Files.Id AND comment.SourceType = 3
                          ...
                          FROM IM_InventoryStateHistoryImage Files with (nolock)
                                   ...
                                   join F_GetPermittedUsers(@userId) Permitted
                                        on Permitted.UserId = History.CreatorUserId
                                   LEFT JOIN MD_PhotoLike Likes with (nolock)
                                             ON Likes.ReferenceId = Files.Id AND Likes.SourceType = 4
                                   left join MD_PhotoComment comment with (nolock)
                                             ON comment.ReferenceId = Files.Id AND comment.SourceType = 4
