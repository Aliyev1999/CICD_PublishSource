create or ALTER   function [dbo].[F_MGM_GetConfirmationOperations](@firm Smallint ,
    @userId int,
    @beginDate datetime,
    @endDate datetime,
    @isCompleted bit
    ...
    returns table
        ...
        return(with Data as (select ILog.Id,
                    ...
                    StatusCalculations as (
                        ...
                        where ILog.DocType in (0, 1, 2, 3, 4) and ILog.Firm=@firm
                        ...
                        from OP_ThirdPartyIncomingLog ILog with (nolock)
                                 ...
                                 left join (select Id, sum(Amount) as Amount
                                            ...
                                            group by Id) LineResultAmount on ILog.Id = LineResultAmount.Id
                        where ILog.DocType in (5, 6) and ILog.Firm=@firm
                        ...
                        from OP_ThirdPartyIncomingLog ILog with (nolock)
                                 ...
                                 left join (select Id, sum(Quantity) as Amount
                                            ...
                                            group by Id) LineResultAmount on ILog.Id = LineResultAmount.Id
                        where ILog.DocType in (21, 22, 23, 24) and ILog.Firm=@firm
