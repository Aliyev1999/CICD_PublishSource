CREATE OR ALTER PROCEDURE [dbo].[SP_MGM_GetPhotoDetail] @firm Smallint, @userId BIGINT,
                                              @likeUserId BIGINT,
                                              @url NVARCHAR(MAX)
AS
BEGIN
    ...
    WITH Data AS (select Clients.Code                           as ClientCode,
                  ...
                  select Clients.Code                           as ClientCode,
                         ...
                         Response.CreatedLatitude               as OperationLatitude
                  from CHL_Attachment Files with (nolock)
                           join CHL_UserDynamicSurveyResponseDetail Reference with (nolock)
                                on Reference.Id = Files.ReferenceId
                           join CHL_UserSurveyResponse Response with (nolock) on Response.Id = UserSurveyResponseId
                           join AbpUsers Users with (nolock) on Users.Id = Response.UserId
                           join MD_Client Clients with (nolock)
                                on Clients.TigerId = Response.ClientId and Clients.Firm = Response.Firm
    ...
    FROM Data
    GROUP BY AttachmentId, SourceType, ClientCode, ClientName, ClientLatitute, ClientLongitude, OperationLatitude,
             OperationLongitude,
