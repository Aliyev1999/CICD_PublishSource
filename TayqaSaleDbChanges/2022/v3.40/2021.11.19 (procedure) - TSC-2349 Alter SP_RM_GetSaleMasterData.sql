ALTER proCEDURE [dbo].[SP_RM_GetSaleMasterData](
    @firm SMALLINT = NULL,
...
BEGIN
    ...
    IF OBJECT_ID('tempdb..#User_SP_RM_UserIds') IS NOT NULL
        DROP TABLE #User_SP_RM_UserIds;

    CREATE TABLE #User_SP_RM_UserIds
    (
        UserId INT NOT NULL PRIMARY KEY
    );

    INSERT INTO #User_SP_RM_UserIds
    SELECT UserId
    FROM F_GetPermittedUsers(@currentUser);
    ...
    IF @forAmount = 0
		...
		from MD_ItemGroup ItemGroup with (nolock)
				 ...
				 join MD_Item Items with (nolock) on Items.TigerId = ItemGroupItems.ItemId and ItemGroupItems.Firm = Items.Firm and
						 Items.IsDeleted = 0 and Items.Firm = @firm and Items.[Status] = 0
				 ...
				 join MD_Client Clients with (nolock) on Invoice.ClientId = Clients.TigerId and Invoice.Firm = Clients.Firm and
														 Clients.IsDeleted = 0 and Clients.[Status] = 0
	...
	where Invoice.SalesmanId IN (SELECT EmployeeId from UIM_UserEmployeeMapping UserEmployee with (nolock) join 
									#User_SP_RM_UserIds PermittedUser on PermittedUser.UserId = UserEmployee.UserId) 
	'
    ELSE
        SET @Query =
                '
