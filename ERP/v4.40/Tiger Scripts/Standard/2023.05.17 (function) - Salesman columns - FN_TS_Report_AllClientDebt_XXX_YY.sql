...
SELECT DebtData.SALESMANREF AS SalesmanId,
...
from DebtData WITH (NOLOCK)
         ...
         INNER JOIN LG_SLSMAN SLSMAN WITH (NOLOCK) ON DebtData.SALESMANREF = SLSMAN.LOGICALREF AND SLSMAN.FIRMNR = CAST(XXX AS INT)
);
