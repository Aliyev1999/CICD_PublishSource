CREATE FUNCTION [dbo].[FN_TS_Report_GetSalesmanFinalizedItemExtract_XXX_YY](@salesmanId INT, @beginDate DATETIME, @endDate DATETIME)
    RETURNS TABLE
        AS RETURN
            (
                SELECT t.ItemCode, t.ItemName, t.ItemId, t.GrossQuantity, t.ReturnQuantity, t.NetQuantity, t.PromoQuantity,
                       --will be deprecated
                       t.GrossQuantity GrossAmount, t.ReturnQuantity ReturnAmount, NetQuantity NetAmount, t.PromoQuantity PromoAmount, 'AZN' As CurrencyCode
                FROM (
                         ...
                         FROM LG_XXX_YY_STLINE LINE WITH (NOLOCK)
                                  ...
                                  INNER JOIN LG_XXX_ITMUNITA mun WITH (NOLOCK) ON mun.ITEMREF = LINE.STOCKREF AND mun.LINENR = (IIF(ITEMS.CARDTYPE = 2, 1, 2))
